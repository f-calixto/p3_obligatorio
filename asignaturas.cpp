...
// Devuelve TRUE si la asignatura existe recorriendo secuencialmente hasta el tope
...
// Elimina una asignatura haciendo un "cambio por el último" para no dejar huecos
void delete_(Asignaturas &A, int numAsig) {
    ...
    while ((i < A.tope) && (!eliminado)) {
        if (obtenerNumero(A.act[i]) == numAsig) {
            // Pisamos el elemento a borrar con el que está en la última posición válida
