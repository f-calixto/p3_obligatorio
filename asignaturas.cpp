...
Asignatura find(Asignaturas A, int numAsig) { return A[numAsig].info; }

// Precondición: la asigantura a sustituir existe en iccionario A.
void modify(Asignaturas& A, Asignatura a, int numAsig) { A[numAsig].info = a; }

// Precondición: la asignatura existe en diccionario A
void delete_(Asignaturas& A, int numAsig) { A[numAsig].existe = FALSE; }
