...
void registrarAsignatura(Asignaturas& asignaturas) {
    if (asignaturas.tope >= MAX_ASIG) {
        printRojo(
            "Error: Se ha alcanzado el tope maximo de asignaturas en el "
            "sistema.\n\n");
    ...
    int tope = largo(asignaturas);
    asignarNumero(nueva, tope);

    if (member(asignaturas, tope)) {
        printRojo(
            "Error: Ya existe una asignatura registrada con ese numero.\n\n");
    } else {
...
void agregarPreviatura(Previaturas& previaturas, Asignaturas asignaturas) {
    int u, v;
    print("Ingrese el numero de la asignatura DESTINO (u): ");
    scanf("%d", &u);
    print("Ingrese el numero de la asignatura PREVIA (v): ");
    scanf("%d", &v);

    // Validación A: Que ambas existan en el sistema
    if (!member(asignaturas, u) || !member(asignaturas, v)) {
        printRojo(
            "Error: Al menos una de las asignaturas especificadas no esta "
            "registrada.\n\n");
        return;  // se usa para que el resto de la función NO se ejecute
    }
    ...
    if (u == v) {
        printRojo(
            "Error: Una asignatura no puede ser previatura de si misma.\n\n");
        return;
    }
    ...
    if (perteneceArista(previaturas, u, v)) {
        printRojo(
            "Error: Esta relacion de previatura ya se encuentra "
            "registrada.\n\n");
        return;  // se usa para que el resto de la función NO se ejecute
    }

    // Validación D: CONTROL DE CICLOS. Si ya hay camino de v a u, agregar u ->
    // v generará un ciclo
    if (hayCamino(previaturas, v, u)) {
        printRojo(
            "Error Invalido: No se puede agregar la previatura porque "
            "generaria un ciclo en el plan de estudios.\n\n");
    } else {
...
void registrarCursoFinalizado(Alumnos& alumnos, Asignaturas asignaturas,
    ...
    if (!streq(nombreCurso, nombreAsig)) {
        printRojo(
            "La asignatura con dicho numero fue ingresada con otro "
            "nombre: ");
...
void listarPrevias(Previaturas previaturas, Asignaturas asignaturas) {
    ...
    if (!member(asignaturas, numAsig)) {
        printRojo(
            "Error: La asignatura especificada no existe en el "
            "sistema.\n\n");
        return;
    }
    ...
    for (int i = 0; i < N; i++) {
        if (i != numAsig && member(asignaturas, i)) {
            if (hayCamino(previaturas, numAsig, i)) {
...
void mostrarEscolaridadAlumno(Alumnos alumnos, Asignaturas asignaturas) {
    ...
    if (!member(alumnos, ci)) {
        printRojo(
            "Error: El alumno con la cedula ingresada no esta "
            "registrado.\n\n");
        return;
    }
    ...
    if (esVacia(esc)) {
        ...
        // recorremos la lista mediante kEsimo desde 1 hasta el largo de la
        // secuencia
        for (int k = 1; k <= cantCursos; k++) {
            ...
            obtenerNombreAsigantura(cursoNodoK, nombreMateria);
